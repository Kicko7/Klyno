...
import { Avatar, Button, Modal, Select, Space, Tag, Typography } from 'antd';
...
const AddMembersModal = memo<AddMembersModalProps>(
  ({ open, onClose, teamChatId, organizationId }) => {
    const { t } = useTranslation('chat');
    const [selectedMembers, setSelectedMembers] = useState<string[]>([]);
    const [loading, setLoading] = useState(false);

    const { teamChatsByOrg, updateTeamChat, currentOrganizationId } = useTeamChatStore();
    ...
    const availableMembers = mockOrgMembers.filter(
      (member) => member.id !== currentUser?.id && !existingMembers.includes(member.id),
    );
        ...
        <Flexbox gap={20}>
          ...
          >
            <Text style={{ fontSize: '12px', color: '#92400e' }}>
              💡 <strong>Note:</strong> Added members will be able to see the entire chat history
              and participate in conversations with the AI assistant.
            </Text>
          </div>
        </Flexbox>
      </Modal>
